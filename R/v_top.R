...
#' @description  V top technical analysis function is to analyze the downward trend of stock data
#' @details use RSI analysis of the strength of the stock market trend, analyze trends conform to v top, and RSI function need library 'TTR'
#' @usage v_top(h,top,down,month,day)
...
#' @return an analysis of stock data for v top technical analysis indicators
...
#hc_add_series(v_top(aapl,60,40,3,20),type = "line",name = "V top", color = hex_to_rgba("green", 1))


v_top<-function(h,top,down,month,day){
        ...
        if(g>1 ){
          if(d5[g,]!=0){
            ...
            if(lotr1!=0 && lotr2!=0 && wength<month && "O" %in% c(d_status[(lotr1+1):(lotr2-1)]) && d5[lotr1]>=d5[lotr2] && lotr1<lotr2 && d_status[lotr1]=="X" && d_status[lotr2]=="X"){
              ...
              #  if((sum(df[((lotr2+1):(lotr2+day))])/day)<down && d1[lotr2+day]<d1[lotr2] && d0[lotr2+day]<d0[lotr2] ){
              #    checkw<-1
              #  }else{
              #    checkw<-0
              #  }
